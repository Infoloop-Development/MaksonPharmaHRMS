...
import { BugReportShareLinkButton } from '../../bugReport/BugReportShareLinkButton';
...
export function BugReportKanbanCard({ item, onOpen, isDragging, shareVariant = 'default' }: Props) {
    ...
    >
      <div className="flex items-start gap-1">
        <button
          ...
          onClick={() => onOpen(item.id, item.publicId)}
        >
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              {item.publicId && (
                <p className="text-[10px] font-mono text-text-muted mb-0.5">{item.publicId}</p>
              )}
