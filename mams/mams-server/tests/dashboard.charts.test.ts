...
describe('getDashboardCharts', () => {
  ...
  it('returns last7Days and day punctuality shape', async () => {
    ...
    expect(result.last7Days.absent.reduce((s, n) => s + n, 0)).toBe(180);
    expect(result.last7Days.late.reduce((s, n) => s + n, 0)).toBe(1);
  ...
  it('queries punctuality for a single date when provided', async () => {
    ...
    const dayPresentMatch = attendanceAggregate.mock.calls[3]?.[0]?.[0]?.$match;
